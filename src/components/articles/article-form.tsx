...
const articleSchema = z.object({
  ...
  precio_2: z.union([z.string(), z.number()]).transform(Number).refine(val => !isNaN(val), { message: "Debe ser un número" }),
  precio_3: z.union([z.string(), z.number()]).transform(Number).refine(val => !isNaN(val), { message: "Debe ser un número" }),
  precio_4: z.union([z.string(), z.number()]).transform(Number).refine(val => !isNaN(val), { message: "Debe ser un número" }),
...
export function ArticleForm({ article, onSave, onCancel, isLoading = false }: ArticleFormProps) {
  ...
  const form = useForm({
    ...
    defaultValues: {
      ...
      precio_2: article?.precio_2 ?? 0,
      precio_3: article?.precio_3 ?? 0,
      precio_4: article?.precio_4 ?? 0,
